  ...
  const html = `
    <html>
      ...
      <body>
        ...
        ${table("Cuotas", ["Nro", "Vencimiento", "Monto", "Monto USD", "Pagada", "Fecha pago"], p.installments.map(c => [c.number, date(c.dueDate), money(c.amount), money(c.amountUSD ?? (isBRL ? 0 : c.amount)), c.paidAt ? "Sí" : "No", date(c.paidAt)]))}
        ${table("Refuerzos", ["Etiqueta", "Vencimiento", "Monto", "Monto USD", "Pagado", "Fecha pago"], p.reinforcements.map(r => [r.label, date(r.dueDate), money(r.amount), money(r.amountUSD ?? (isBRL ? 0 : r.amount)), r.paidAt ? "Sí" : "No", date(r.paidAt)]))}
      </body>
    </html>
  `
